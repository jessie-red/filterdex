import { ScryDex, allSpecies, allFormes, allAbilities, allMoves } from "./dex.ts";
import { matchWord, normalize, levenshtein, MIN_SCORE } from "./match.ts";
import type { WordMatch } from "./match.ts";
import { types, tiers, stages, eggGroups, regions, tags, colors } from "./pokemon.ts";
...
const FILTER_FUNCTIONS: getFilterFunction[] = [getSpeciesFilter, getTypeFilter, getAbilityFilter, getMoveFilter, getTierFiler, getRegionFilter, getFormeFilter, getStageFilter, getEggGroupFilter, getTagFilter, getColorFilter, getLegalFilter, getHpFilter, getAtkFilter, getDefFilter, getSpaFilter, getSpdFilter, getSpeFilter, getGenFilter, getWeightFilter, getHeightFilter, getNumFilter, getBstFilter];
...
function tryGetFilter(words: string[]): FilterMatch | null {
    ...
    for (const filterFunction of FILTER_FUNCTIONS) {
        const match: FilterMatch | null = filterFunction(words.join('').replaceAll(' ', '').toLowerCase());
        if (match) {
            if (match.match === 100) {
    ...
    if (best) console.log(`Best match for "${words.join(' ')}" is ${best.match}%`);
    return best;
}

function stripPrefix(word: string, prefixes: string[]): { value: string; prefixed: boolean } {
...
function getSpeciesFilter(word: string): FilterMatch | null {
    const { value, prefixed } = stripPrefix(word, ['name', 'species', 'pokemon', 's']);
...
function getAbilityFilter(word: string): FilterMatch | null {
    ...
    return {
        filter: {
            filter: (p) => Object.values(p.abilities).some(a => a && a.toLowerCase().replace(/[^a-z0-9]/g, '') === match.word),
...
function getMoveFilter(word: string): FilterMatch | null {
    ...
    return {
        filter: {
            filter: (p) => p.moves.some(m => m.toLowerCase().replace(/[^a-z0-9]/g, '') === match.word),
...
function getTierFiler(word: string): FilterMatch | null {
    ...
    return {
        filter: {
            filter: (p) => p.tier.toLowerCase().replace(/[^a-z0-9]/g, '') === match.word,
...
function getFormeFilter(word: string): FilterMatch | null {
    ...
    return {
        filter: {
            filter: (p) => p.forme !== undefined && p.forme.toLowerCase().replace(/[^a-z0-9]/g, '') === match.word,
...
function getStageFilter(word: string): FilterMatch | null {
    ...
    return {
        filter: {
            filter: (p) => p.stage.toLowerCase().replace(/[^a-z0-9]/g, '') === match.word,
...
function getEggGroupFilter(word: string): FilterMatch | null {
    ...
    return {
        filter: {
            filter: (p) => p.eggGroups.some(eg => eg.toLowerCase().replace(/[^a-z0-9]/g, '') === match.word),
...
function getRegionFilter(word: string): FilterMatch | null {
    ...
    return {
        filter: {
            filter: (p) => p.region.toLowerCase().replace(/[^a-z0-9]/g, '') === match.word,
...
function getTagFilter(word: string): FilterMatch | null {
    ...
    return {
        filter: {
            filter: (p) => p.tags.some(t => t.toLowerCase().replace(/[^a-z0-9]/g, '') === match.word),
...
function getColorFilter(word: string): FilterMatch | null {
    ...
    return {
        filter: {
            filter: (p) => p.color.toLowerCase().replace(/[^a-z0-9]/g, '') === match.word,
...
function parseNumericFilter(word: string, prefixes: string[]): { op: Operator; value: number } | null {
    for (const prefix of prefixes) {
...
function makeNumericFilter(word: string, prefixes: string[], label: string, getter: (p: Pokemon) => number): FilterMatch | null {
...
function getSpaFilter(word: string): FilterMatch | null {
    return makeNumericFilter(word, ['spa'], 'special attack', p => p.baseStats.spa);
}

function getSpdFilter(word: string): FilterMatch | null {
    return makeNumericFilter(word, ['spd'], 'special defense', p => p.baseStats.spd);
}
...
function getNumFilter(word: string): FilterMatch | null {
    return makeNumericFilter(word, ['num', 'n', 'dex'], 'dex number', p => p.num);
}

function getBstFilter(word: string): FilterMatch | null {
    return makeNumericFilter(word, ['bst'], 'base stat total', p => p.baseStats.hp + p.baseStats.atk + p.baseStats.def + p.baseStats.spa + p.baseStats.spd + p.baseStats.spe);
}
