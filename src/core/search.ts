...
function extractSort(words: string[]): { getter: (p: Pokemon) => number; desc: boolean } | null {
...
export function searchPokemon(query: string): SearchResult {
    ...
    const results = ScryDex.filter(p => filterSet.filters.every(f => f.filter(p)));
    const fuzzyFilter = filterSet.filters.find(f => f.desc?.includes("like"));
    if (sort) {
        results.sort((a, b) => sort.desc ? sort.getter(b) - sort.getter(a) : sort.getter(a) - sort.getter(b));
    } else if (fuzzyFilter) {
        results.sort((a, b) => (fuzzyFilter.filter(b) as number) - (fuzzyFilter.filter(a) as number));
    }
    return { results, desc };
}
