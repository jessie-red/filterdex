...
import type { Pokemon, Type, Stage, Region, EggGroup, Tag, Color } from './pokemon.ts';
...
const CHAMPIONS_URL = 'https://raw.githubusercontent.com/smogon/pokemon-showdown/master/data/mods/champions';
...
function determineVGCRules(format: string): VGCRules {
    ...
    const minsourcegen = ruleset.find(rule => rule.startsWith('Min Source Gen'))?.split('=')[1] || "1";
...
async function fetchChampionsData(): Promise<ChampionsData | null> {
    try {
        ...
        for (const match of formatsText.matchAll(/\t(\w+):\s*\{([\s\S]*?)\n\t\}/g)) {
        ...
        for (const [, pokemonId, content] of learnsetsText.matchAll(/\t(\w+):\s*\{\s*learnset:\s*\{([\s\S]*?)\}\s*,?\s*\}/g)) {
            const moves: string[] = [];
...
async function buildDex(): Promise<DexData> {
    ...
    for (const species of Dex.species.all()) {
        ...
        for (const a of Object.values(megaOverride?.abilities ?? species.abilities)) {
        ...
        const pokemon: Pokemon = {
            ...
            types: [parseType(species.types[0]), species.types[1] ? parseType(species.types[1]) : undefined],
            moves,
...
function getMoves(species: Species, champions: ChampionsData | null): string[] {
    const baseId = species.baseSpecies ? Dex.species.get(species.baseSpecies).id : species.id;
    const learnset = champions?.learnsets.get(species.id) ?? champions?.learnsets.get(baseId);
    if (learnset) {
        return learnset
    ...
    return [...Dex.species.getMovePool(species.id)].map(m => Dex.moves.get(m).name);
}
...
const { dex: ScryDex, allSpecies, allFormes, allAbilities, allMoves } = await buildDex();
